...
while getopts 'cd:h' opt; do
    case "$opt" in
        c)
            ...
            test -e /dev/video10 && LOOPCAM=--device=/dev/video10:/dev/video10
            echo $CAM0 $CAM1 $CAM2 $CAM3 $LOOPCAM
            ;;
        d)
            DATA_FOLDER="$OPTARG"
...
xhost +
docker run --ipc host --gpus all -e DISPLAY=$DISPLAY -v /tmp/.X11-unix:/tmp/.X11-unix -v $DATA_FOLDER:/data/ $CAM0 $CAM1 $CAM2 $CAM3 $LOOPCAM  --rm -it deepfacelive
