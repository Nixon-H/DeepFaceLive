...
class QStreamOutput(QBackendPanel):
    def __init__(self, backend : StreamOutput):
        ...
        q_virtual_cam_label = QLabelPopupInfo(label='Virtual Cam (/dev/video10)')
        q_virtual_cam       = QCheckBoxCSWFlag(cs.is_virtual_cam, reflect_state_widgets=[q_virtual_cam_label])
        ...
        row += 1
        grid_l.addLayout( qtx.QXHBoxLayout([q_virtual_cam, 4, q_virtual_cam_label]), row, 0, 1, 3, alignment=qtx.AlignLeft | qtx.AlignVCenter )
        row += 1
