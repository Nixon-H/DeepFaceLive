...
class StreamOutputWorker(BackendWorker):
    ...
    def on_start(self, weak_heap : BackendWeakHeap, reemit_frame_signal : BackendSignal,
        ...
        cs.is_virtual_cam.call_on_flag(self.on_cs_is_virtual_cam)
        ...
        cs.is_virtual_cam.enable()
        cs.is_virtual_cam.set_flag(state.is_virtual_cam if state.is_virtual_cam is not None else False)
    ...
    def on_cs_is_virtual_cam(self, is_virtual_cam):
        state, cs = self.get_state(), self.get_control_sheet()
        state.is_virtual_cam = is_virtual_cam
        if is_virtual_cam:
            self._open_v4l2()
        else:
            self._close_v4l2()
        ...
        if bcd is not None:
            ...
            if source_type is not None and \
                (state.is_showing_window or \
                 ...
                 state.is_streaming or \
                 state.is_virtual_cam):
...
class Sheet:
    class Host(lib_csw.Sheet.Host):
        def __init__(self):
            ...
            self.is_virtual_cam = lib_csw.Flag.Client()
    ...
    class Worker(lib_csw.Sheet.Worker):
        def __init__(self):
            ...
            self.is_virtual_cam = lib_csw.Flag.Host()
